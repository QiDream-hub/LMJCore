...
> 📝 **命名说明**：  
> `LMJCORE_SET` 对应传统概念中的数组，但强调其**无序集合**的本质。如需有序列表，请在值中编码顺序信息。
...
|属性|说明|
|---|---|
|**职责**|存储实体的关联项列表（成员名或集合元素）|
|**Key**|实体指针（17B，对象或集合均可）|
|**Value**|成员名（对象）或元素值（集合），可为原始数据或指针|
|**重复 Key**|允许（启用 `MDB_DUPSORT`）|
|**排序行为**|**按 Value 字典序自动排序**，**不保留插入顺序**|
|**唯一性**|同一 Key 下，Value 自动去重（LMDB DUPSORT 特性）|
|**随机访问**|不支持下标访问|

> 📌 **命名说明**：  
> `set` 数据库的名称准确反映了其用途——存储实体的“集合”关系。
...
|状态|判定条件|
|---|---|
|**有效实体**|指针在 `set` 中存在至少一个条目|
|**无效指针**|`set` 中无对应 Key|
|**空实体**|`set` 中存在 Key，但无任何 Value（合法状态）|
...
- **成员定义**：当且仅当该成员名出现在该对象指针对应的 `set` 条目中。
- **已赋值**：`main` 中存在对应 Key-Value。
- **缺失值（Missing Value）**：`set` 中有成员名，但 `main` 中无值 → **合法中间状态**（如分步初始化）。
...
- **定义**：`main` 中存在某成员的值，但其成员名**未在 `set` 中注册**。
...
- 同一集合中，相同元素值只能出现一次（由 LMDB DUPSORT 自动保证）。
- 如需存储重复元素，应在值中编码区分信息（如计数器、时间戳）。
...
|操作对象|创建|读取|更新|删除|存在性检查|
|---|---|---|---|---|---|
|**对象**|`lmjcore_obj_create`|`lmjcore_obj_member_get`|`lmjcore_obj_member_put`|`lmjcore_obj_member_del`|`lmjcore_obj_member_exist`|
|**集合**|`lmjcore_set_create`|`lmjcore_set_member_get`|`lmjcore_set_member_add`|`lmjcore_set_member_del`|`lmjcore_set_member_exist`|

> 📝 **命名说明**：  
> - 集合的“添加”操作用 `add` 而非 `put`，强调其集合语义（无序、自动去重）
> - 对象的“更新”操作用 `put`，强调键值对语义
...
如果代码中使用了旧的 `LMJCORE_ARR` 类型和 `arr` 数据库：

1. **类型迁移**：`LMJCORE_ARR` → `LMJCORE_SET`
2. **数据库迁移**：`"arr"` → `"set"`
3. **API 迁移**：
   - `lmjcore_arr_create` → `lmjcore_set_create`
   - `lmjcore_arr_append` → `lmjcore_set_member_add`
   - `lmjcore_arr_remove` → `lmjcore_set_member_del`
   - `lmjcore_arr_iterate` → `lmjcore_set_iterate`

> 💡 内核版本 2.0 起采用新命名，旧 API 将在后续版本中移除。
