...
void print_test_result(const char *test_name, int result) {
    printf("%-40s: %s\n", test_name, 
           result == LMJCORE_SUCCESS ? "PASS" : "FAIL");
}
...
int test_basic_object_operations(lmjcore_env *env) {
    ...
    ret = lmjcore_obj_get(txn, obj_ptr, LMJCORE_MODE_LOOSE, 
    ...
    if (ret == LMJCORE_SUCCESS) {
        ...
        for (size_t i = 0; i < member_count; i++) {
            ...
            ret = lmjcore_parser_obj_get_member(result, result_buf, i,
                                             &name_data, &name_len,
                                             &value_data, &value_len);
            if (ret == LMJCORE_SUCCESS) {
...
int test_array_operations(lmjcore_env *env) {
    ...
    for (int i = 0; i < 4; i++) {
        ret = lmjcore_arr_append(txn, arr_ptr,
                                (const uint8_t*)elements[i], strlen(elements[i]));
        printf("添加元素 '%s': %s\n", elements[i], 
    ...
    ret = lmjcore_arr_get(txn, arr_ptr, LMJCORE_MODE_LOOSE, 
                         result_buf, sizeof(result_buf), &result);
    ...
    if (ret == LMJCORE_SUCCESS) {
        ...
        for (size_t i = 0; i < element_count; i++) {
            ...
            ret = lmjcore_parser_arr_get_element(result, result_buf, i,
        ...
        // 测试数组查找
        size_t found_index;
        ret = lmjcore_parser_arr_find_element(result, result_buf,
...
int test_error_handling(lmjcore_env *env) {
    ...
    // 设置一个正常成员
    ret = lmjcore_obj_member_put(txn, obj_ptr, 
                                (const uint8_t*)"normal_field", 12,
                                (const uint8_t*)"has_value", 9);
    ...
    printf("测试严格模式:\n");
    ret = lmjcore_obj_get(txn, obj_ptr, LMJCORE_MODE_STRICT, 
                         result_buf, sizeof(result_buf), &result);
...
int test_edge_cases(lmjcore_env *env) {
    ...
    // 测试不存在的对象
    ret = lmjcore_obj_get(txn, empty_obj, LMJCORE_MODE_LOOSE, 
                         result_buf, sizeof(result_buf), &result);
    if (ret == LMJCORE_ERROR_ENTITY_NOT_FOUND) {
        printf("正确报告不存在的对象\n");
    }
